...
//			mongoDb.createCollection("trxs", new BasicDBObject());
			...
			System.out.println(meridsColl.findOne());
...
//			System.out.println(dbo);
	...
	private static void countTrx()
	{
		  String map = "function() { " +
	                 "  	for(element in this) " +
	                 "		{ " +
	                 "			if (this[element]['trxs'] != null) " +
	                 "			{" +
	                 "				emit('count', this[element].trxs.length); " +
	                 "			}" +
	                 "		}" +
	                 "}";
		  String reduce = "function(key, values) { " +
                  ...
                  "}";
		  MapReduceCommand cmd = new MapReduceCommand(meridsColl, map, reduce,
		             null, MapReduceCommand.OutputType.INLINE, null);
		  MapReduceOutput out = meridsColl.mapReduce(cmd);
